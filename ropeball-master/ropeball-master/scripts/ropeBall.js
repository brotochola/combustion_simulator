class RopeBallEngine {
  ...
  removeFromChunk(i) {
    this.chunks[this.balls.cx[i]][this.balls.cy[i]].splice(
      this.chunks[this.balls.cx[i]][this.balls.cy[i]].indexOf(i),
      1
    );
  }
  addBall(x, y, dx, dy) {
  ...
  doConstrants() {
    ...
    for (let s = 0; s < this.subStebCount; s++) {
      for (let i = 0; i < this.balls.activeCount; i++) {
        ...
        if (this.balls.x[i] < this.ballRadius)
          this.balls.x[i] = this.ballRadius;
        if (this.balls.y[i] < this.ballRadius)
          this.balls.y[i] = this.ballRadius;
        if (this.balls.x[i] > this.width - this.ballRadius)
          this.balls.x[i] = this.width - this.ballRadius;
        if (this.balls.y[i] > this.height - this.ballRadius)
          this.balls.y[i] = this.height - this.ballRadius;
