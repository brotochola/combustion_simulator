...
Renderer.prototype.insertCircleVertices = function (
  transform,
  radius,
  x,
  y,
  ...
  for (var i = 0; i < this.circleResolution; i++) {
    var i4 = i * 4;
    var v1 = new b2Vec2(
      vertices[i4] * radius + x,
      vertices[i4 + 1] * radius + y
    );
    var v2 = new b2Vec2(
      vertices[i4 + 2] * radius + x,
      vertices[i4 + 3] * radius + y
    );
...
Renderer.prototype.transformVerticesAndInsert = function (
  vertices,
  transform,
  r,
  g,
...
function drawParticleSystem(system) {
  ...
  for (var i = 0, c = 0; i < maxParticles; i += 2, c += 4) {
    renderer.insertParticleVertices(
      system.radius,
      particles[i],
      particles[i + 1],
      color[c] * inv255,
...
function getParticle(x, y) {
  ...
  for (var i = 0; i < maxParticles; i += 2) {
    let diffX = Math.abs(particles[i] - x);
    let diffY = Math.abs(particles[i + 1] - y);
    if (diffX < system.radius * 0.5 && diffY < system.radius * 0.5) {
      return i;
    }
    // system.radius,
    // particles[i],
